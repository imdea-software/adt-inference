...
\emph{sequential}. Two operations with the same labels and matching are
\emph{identical}, and the \emph{frequency} of an operation $o$, denoted
$\freq(o)$, is the number of operations identical to $o$, including $o$ itself.
An operation is a \emph{duplicate} when $\freq(o) > 1$. An operation $o \in
\rng(m)$ is a \emph{match target}, and the set $\set{ o' \in O : m(o') = o}$ of
operations targeting $o$ is a \emph{match}. We assume targets target
themselves: $m(o) = o$ for $o \in \rng(m)$. The \emph{weight} of $h$ is the
maximum $\max(\set{ \freq(o) : o \in O } \cup \set{|\rng(m)|})$ between
operation frequencies and the number of matches. Since the operation
identifiers of a history have no intrinsic meaning, we consider equality
between histories up to renaming of operation identifiers.
